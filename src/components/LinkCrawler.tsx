...
import { ShieldCheck, ShieldAlert, Link2, Search, Filter, Ban, CheckCircle2, Clock, Globe, Download } from 'lucide-react';
import axios from 'axios';
import { clsx } from 'clsx';
import { generateCrawlerPDF } from '../utils/pdf';
...
export default function LinkCrawler({ links, setLinks }: LinkCrawlerProps) {
  ...
  const [config, setConfig] = useState({ systemName: 'UptimeMonitor', companyName: 'My Company' });
    ...
    const fetchConfig = async () => {
      try {
        ...
        if (res.data.settings) {
          setConfig({
            systemName: res.data.settings.systemName || 'UptimeMonitor',
            companyName: res.data.settings.companyName || 'My Company'
          });
        }
      } catch (err) {}
    };
  ...
  const handleDownloadPDF = () => {
    generateCrawlerPDF(links, {
      systemName: config.systemName,
      companyName: config.companyName,
      reportTitle: 'Link Crawler Report'
    });
  };
