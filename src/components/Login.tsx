...
export default function Login({ onLoginSuccess }: LoginProps) {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      onLoginSuccess(res.data.token, res.data.roles || []);
    } catch (err: any) {
  ...
  const handleRegister = async (e: FormEvent) => {
    ...
    setError('');
    try {
      const res = await axios.post('/api/auth/register', { name, companyName, email, password });
      onLoginSuccess(res.data.token, res.data.roles || []);
    } catch (err: any) {
        ...
        <div className="bg-slate-900 py-8 px-4 shadow sm:rounded-xl border border-slate-800 sm:px-10">
          ...
          {view === 'login' && (
            <form className="space-y-6" onSubmit={handleLogin}>
              ...
              <div className="text-center text-sm">
                ...
                <button type="button" onClick={() => { setView('register'); setError(''); setMessage(''); }} className="font-medium text-indigo-400 hover:text-indigo-300">
                  Register here
                </button>
              </div>
            </form>
          ...
          {view === 'register' && (
            <form className="space-y-6" onSubmit={handleRegister}>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300" htmlFor="companyName">
                  Company Name
                </label>
                <div className="mt-1">
                  <input
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300" htmlFor="email-register">
                  Email address
                </label>
                <div className="mt-1">
                  <input
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300" htmlFor="password-register">
                  Password
                </label>
                <div className="mt-1">
                  <input
              ...
              <div className="text-center text-sm">
                ...
                <button type="button" onClick={() => { setView('login'); setError(''); setMessage(''); }} className="font-medium text-indigo-400 hover:text-indigo-300">
                  Sign in
                </button>
              </div>
            </form>
